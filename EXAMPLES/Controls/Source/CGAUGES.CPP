...
void __fastcall TCGauge::SetMinValue(Longint Value)
{
  if (Value!= FMinValue){
    ...
    {
      String message;
      throw Exception(message.sprintf(Consts_SOutOfRange.c_str(),-MaxInt,FMaxValue - 1));
    }
    FMinValue = Value;
    if (FCurValue < Value)
      FCurValue = Value;
...
void __fastcall TCGauge::SetMaxValue(Longint Value)
{
  if (Value){
    ...
    {
      String message;
      throw Exception(message.sprintf(Consts_SOutOfRange.c_str(),FMinValue + 1,MaxInt));
    }
