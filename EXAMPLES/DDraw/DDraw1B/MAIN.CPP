...
/* Remember: You have two surfaces. When you erase the old image, you want to
   think back two iterations, not just one. So I don't erase the place
   where it was last time the function was called, but the last time
   minus one. In other words, one surface moves the ball two spaces forward,
   and there are two surfaces, so you erase the position two calls back
   (four spaces) for each surface. */
