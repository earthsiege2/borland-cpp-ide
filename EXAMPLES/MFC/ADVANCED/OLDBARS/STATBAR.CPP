...
void CStatusBar::OnWinIniChange(LPCTSTR /*lpszSection*/)
{
	if (globalData.bWin4)
	{
		...
		m_cxSizeBox = std::min(GetSystemMetrics(SM_CXVSCROLL)+1, rect.Height());
	}
}
...
void CStatusBar::DoPaint(CDC* pDC)
{
	...
	if (cxSizeBox != 0)
	{
		int cxMax = std::min(cxSizeBox, rect.Height()+m_cyTopBorder);
...
UINT CStatusBar::OnNcHitTest(CPoint point)
{
	...
	if (!m_bHideSizeBox && m_cxSizeBox != 0)
	{
		...
		GetClientRect(rect);
		CalcInsideRect(rect, TRUE);
		int cxMax = std::min(m_cxSizeBox-1, rect.Height());
