...
void CThreadView::OnKillThreads()
{
	...
	while (!m_threadList.IsEmpty())
	{
		CGDIThread* pThread = m_threadList.RemoveHead();
                /* Don't use the assertion check here since it's likely that the
                   thread has already terminates, causing WaitForSingleObject
                   to fail with an invalid thread handle. */
