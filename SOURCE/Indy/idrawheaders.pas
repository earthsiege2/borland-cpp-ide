...
{$ENDIF}
    ipopt_list: array[0..Id_MAX_IPOPTLEN - 1] of char;
  end;
  ...
  TIdIpHdr = packed record
    ip_verlen: byte;
      // 1st nibble version, 2nd nibble header length div 4 (little-endian)
    ip_tos: byte; // type of service
    ip_len: word; // total length
    ip_id: word; // identification
    ip_off: word;
      // 1st nibble flags, next 3 nibbles fragment offset (little-endian)
...
const
  // IGMP types
  Id_IGMP_MEMBERSHIP_QUERY = $11;
  Id_IGMP_V1_MEMBERSHIP_REPORT = $12;
  Id_IGMP_V2_MEMBERSHIP_REPORT = $16;
  Id_IGMP_LEAVE_GROUP = $17;
