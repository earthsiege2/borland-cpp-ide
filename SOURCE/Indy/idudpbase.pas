...
type
  ...
  protected
    ...
    procedure SetBroadcastFlag(const Enabled: Boolean; ABinding: TIdSocketHandle
      = nil);
  ...
  public
    ...
    function ReceiveBuffer(var ABuffer; const ABufferSize: Integer; var VPeerIP:
      string;
      var VPeerPort: integer; AMSec: Integer = IdTimeoutDefault): integer;
        overload;
    function ReceiveString(const AMSec: Integer = IdTimeoutDefault): string;
      overload;
    ...
    property ReceiveTimeout: Integer read FReceiveTimeout write FReceiveTimeout
      default IdTimeoutInfinite;
  published
    property Active: Boolean read GetActive write SetActive default False;
    property BufferSize: Integer read FBufferSize write FBufferSize default
      ID_UDP_BUFFERSIZE;
    property BroadcastEnabled: Boolean read FBroadcastEnabled write
      SetBroadcastEnabled default False;
  end;
...
procedure TIdUDPBase.Send(AHost: string; const APort: Integer; const AData:
  string);
begin
  SendBuffer(AHost, APort, PChar(AData)^, Length(AData));
end;

procedure TIdUDPBase.SendBuffer(AHost: string; const APort: Integer; var
  ABuffer;
  const AByteCount: integer);
begin
...
begin
  if FBroadCastEnabled <> Value then
  begin
...
procedure TIdUDPBase.SetBroadcastFlag(const Enabled: Boolean; ABinding:
  TIdSocketHandle = nil);
var
...
begin
  ...
  ABinding.SetSockOpt(Id_SOL_SOCKET, Id_SO_BROADCAST, PChar(@BroadCastData),
