...
type
  ...
  TAccessFileEvent = procedure(Sender: TObject; var FileName: string; const
    PeerInfo: TPeerInfo;
  ...
  protected
    ...
    procedure DoWriteFile(FileName: string; const Mode: TIdTFTPMode;
      const PeerInfo: TPeerInfo; RequestedBlockSize: Integer = 0); virtual;
    procedure DoTransferComplete(const Success: Boolean; const PeerInfo:
      TPeerInfo; SourceStream: TStream; const WriteOperation: Boolean); virtual;
  ...
  published
    ...
    property OnTransferComplete: TTransferCompleteEvent read FOnTransferComplete
...
type
  ...
  public
    constructor Create(AnOwner: TIdTrivialFTPServer; const Mode: TIdTFTPMode;
      const PeerInfo: TPeerInfo;
      AStream: TStream; const FreeStreamOnTerminate: boolean; const
        RequestedBlockSize: Integer = 0); virtual;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SourceStream := TFileStream.Create(FileName, fmOpenRead or
          fmShareDenyWrite);
        FreeOnComplete := True;
      end;
...
procedure TIdTrivialFTPServer.DoTransferComplete(const Success: Boolean;
  const PeerInfo: TPeerInfo; SourceStream: TStream; const WriteOperation:
    Boolean);
begin
...
begin
  ...
  try
    if Assigned(FOnWriteFile) then
      FOnWriteFile(Self, FileName, PeerInfo, CanWrite, DestinationStream,
        FreeOnComplete);
    if CanWrite then
    begin
      if DestinationStream = nil then
      begin
        DestinationStream := TFileStream.Create(FileName, fmCreate or
          fmShareDenyWrite);
        FreeOnComplete := True;
      end;
...
begin
  case PosInStrArray(mode, ['octet', 'binary', 'netascii'], False) of
    0, 1: Result := tfOctet;
    2: Result := tfNetAscii;
  else
    raise EIdTFTPIllegalOperation.CreateFmt(RSTFTPUnsupportedTrxMode, [mode]);
      // unknown mode
  end;
end;
...
begin
  ...
  begin
    FUDPClient.BufferSize := Max(FRequestedBlkSize + hdrsize,
      FUDPClient.BufferSize);
    Response := WordToStr(GStack.WSHToNs(TFTP_OACK)) + 'blksize'#0
  ...
  try
    ...
    begin
      ...
      if RetryCtr = 3 then
        raise EIdTFTPIllegalOperation.Create(RSTimeOut);
      ...
      case GStack.WSNToHs(StrToWord(Buffer)) of
        ...
        TFTP_DATA:
          begin
            ...
            begin
              FStream.WriteBuffer(Buffer[hdrsize + 1], Length(Buffer) -
                hdrsize);
              Response := '';
              BlkCounter := Word(succ(BlkCounter));
            end;
    ...
    on E: EWriteError do
      SendError(FUDPClient, ErrAllocationExceeded, Format(RSTFTPDiskFull,
        [FStream.Position]));
    on E: Exception do
      SendError(FUDPClient, E);
  end;
...
begin
  ...
  FOwner.DoTransferComplete(EOT, PeerInfo, FStream, self is
