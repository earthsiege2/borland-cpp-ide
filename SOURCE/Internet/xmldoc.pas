...
{ TXMLNode }
  ...
  protected
    ...
    property HostedNodes: TList read GetHostedNodes;
...
begin
  case NodeType of
    ...
    ntComment: Result :=
      Doc.createComment(NameOrData);
    ntDocFragment: Result :=
...
begin
  ...
  FIsDocElement := False;
  FHostNode := HostNode;
  FHostNode.HostedNodes.Add(Self);
end;
...
begin
  ...
  begin
    for I := 0 to FHostedNodes.Count - 1 do
      TXMLNode(FHostedNodes[I]).FHostNode := nil;
    FreeAndNil(FHostedNodes);
  end;
  if FHostNode <> nil then
    FHostNode.HostedNodes.Remove(Self);
end;
...
begin
  ...
  begin
    ...
    if Assigned(FHostedNodes) then
      for I := 0 to FHostedNodes.Count - 1 do
        TXMLNode(FHostedNodes[I]).SetAttributeNodes(Value);
  end;
end;
...
begin
  ...
  if Assigned(FHostedNodes) then
    for I := 0 to FHostedNodes.Count - 1 do
    begin
      HostedNode := TXMLNode(FHostedNodes[I]);
...
begin
  ...
  begin
    ...
    if Assigned(FHostedNodes) then
      for I := 0 to FHostedNodes.Count - 1 do
        TXMLNode(FHostedNodes[I]).SetChildNodes(Value);
  end;
end;
...
begin
  ...
  for I := 0 to AttributeNodes.Count - 1 do
    if SameNamespace(VarToStr(AttributeNodes[I].NodeValue), NamespaceURI) and
       (AttributeNodes[I].Prefix = SXMLNS) then
    begin
...
begin
  ...
  if Assigned(FHostedNodes) then
    for I := 0 to FHostedNodes.Count - 1 do
      if TObject(FHostedNodes[I]) is NodeClass then
      begin
        Result := TXMLNode(FHostedNodes[I]);
        Break;
      end;
end;
...
begin
  ...
  begin
    ...
    (DOMNode.childNodes[0].nodeType <> TEXT_NODE) then
...
begin
  Result := ((DOMNode.childNodes.length = 1) and
...
function TXMLNode.GetHostedNodes: TList;
begin
  if not Assigned(FHostedNodes) then
    FHostedNodes := TList.Create;
  Result := FHostedNodes;
end;
...
begin
  ...
  FPrefixID := 0;
  SetModified(False);
  if DocSource <> xdsXMLProperty then
    SetXMLStrings('');
end;
