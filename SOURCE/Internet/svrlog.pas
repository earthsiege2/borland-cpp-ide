...
uses WinSock, HTTPApp, Classes, SyncObjs, Windows, Messages,
  ScktComp, SvrConst;
...
type
  ...
  private
    ...
    procedure SetRequestTime(AThreadID: DWORD; AStartTime: TDateTime);
    function GetRequestTime(AThreadID: DWORD): TDateTime;
    function FindRequestTime(AThreadID: DWORD): PRequestTime;
  protected
  ...
  public
    ...
    procedure LogResponse(const Buffer: string);
    property RequestTime[AThreadID: DWORD]: TDateTime read GetRequestTime write SetRequestTime;
  end;
...
uses SysUtils, Forms, SvrHTTP, HTTPParse;
...
procedure TCustomServerLog.LogError(AErrorEvent: TErrorEvent;
...
procedure TCustomServerLog.LogRequest(Request: TWebRequest);
begin
  LogTransaction(TRequestTransaction.Create(Request));
end;

procedure TCustomServerLog.LogResponse(const Buffer: string);
begin
  LogTransaction(TResponseTransaction.Create(Buffer));
end;

function TCustomServerLog.GetRequestTime(AThreadID: DWORD): TDateTime;
var
  P: PRequestTime;
begin
...
function TCustomServerLog.FindRequestTime(AThreadID: DWORD): PRequestTime;
var
  I: Integer;
begin
...
procedure TCustomServerLog.SetRequestTime(AThreadID: DWORD;
...
type
  TWebServerRequestCracker = class(TWebServerRequest);

constructor TRequestTransaction.Create(ARequest: TWebRequest);
var
  R: TWebServerRequestCracker;
begin
  inherited Create;
  FDateTime := Now;
  ...
  R := TWebServerRequestCracker(ARequest as TWebServerRequest);
  FBuffer := R.RequestBuffer;
end;
