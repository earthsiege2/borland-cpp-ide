...
/*----------------------------------------------------------------------
 * The default RTL exception handler
 */
...
Name            DefHandler - default Exception handler

Usage           ULONG __EHCC DefHandler(
...
static ULONG __EHCC DefHandler(PEXCEPTIONREPORTRECORD p,
							   PEXCEPTIONREGISTRATIONRECORD q,
							   PCONTEXTRECORD r,
							   PVOID s)
{
...
Description     This function is the sole exception handler for the RTL.
                It merely calls the exception handler pointed to
                by _UserHandlerPtr.  If _UserHandlerPtr cannot handle
                the exception, the exception handler pointed by
                _HandlerPtr is called.  Normally, both _UserHandler and
                _HandlerPtr point to DefHandler, the default handler.
                The user program can modify _UserHandlerPtr by calling
                _SetUserHandler. If signal() is used, _HandlerPtr will be
                modified to point to signal()'s own exception handler.

Return value    This function returns the value returned
                by the function pointed to by _UserHandlerPtr (if it
                returns XCPT_CONTINUE_EXCECUTION), or the value
                returned by _HandlerPtr.
...
{
    _UserHandlerPtr = DefHandler;   /* use default handler again */
}
...
#pragma startup _init_except 0
    ...
    _HandlerPtr = DefHandler;       /* point to default signal handler */
    _UserHandlerPtr = DefHandler;   /* point to default user handler */
}
