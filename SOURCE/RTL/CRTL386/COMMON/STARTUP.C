...
)
{
     ...
     * We have to scan down to find the actual base, because LINK386
     * doesn't set the stack_start label (in C02) to the base.
     */
    _stkbase = mod_table->stackbase;
#if 0
    for (size = 4096;
         DosQueryMem((void *)_stkbase, &size, &flags) == 0 &&
         (flags & PAG_BASE) == 0;
         _stkbase -= 4096)
        ;
#endif
    ...
    /* Set up global variables pointing to the the environment data
