...
int  _cdecl _EXPFUNC _rtl_chmod (const char *filename, int func, ...)
{
    DWORD attr;
    ...
    {
        if ((attr = GetFileAttributes((LPSTR)filename)) == -1)
            return (__NTerror());
    }
...
int  _cdecl _EXPFUNC _chmod (const char *filename, int func, ...)
{
    DWORD attr;
