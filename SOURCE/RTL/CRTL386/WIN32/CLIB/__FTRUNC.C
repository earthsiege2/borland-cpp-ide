...
int _RTLENTRY _EXPFUNC __ftruncate(int handle, off_t size)
{
     ...
     */
    if ((filesize = GetFileSize(hfile,NULL)) == -1)
        RETURN (__NTerror());

    /* Get the current file position (in filepos).
     */
    if ((filepos = SetFilePointer(hfile, 0L, NULL, FILE_CURRENT)) == -1)
        RETURN (__NTerror());

    /* Set the new file size.
     */
    if (SetFilePointer(hfile, (DWORD)size, NULL, FILE_BEGIN) == -1)
        RETURN (__NTerror());
    ...
    if ((DWORD)size > filesize)
    {
         ...
         */
        if ((newptr = SetFilePointer(hfile, filesize, NULL, FILE_BEGIN)) == -1)
            RETURN (__NTerror());
     ...
     */
    if (SetFilePointer(hfile, filepos, NULL, FILE_BEGIN) == -1)
        RETURN (__NTerror());
    rc = 0;
