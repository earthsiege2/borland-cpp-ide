 ...
 *        __fnsplit - splits a full path name into its components
...
Name            __fnsplit - splits a full path name into its components

Usage           #include <dir.h>
                int __fnsplit(const char *path, char * drive, char * dir,
                             char * name, char * ext);
...
Description     __fnsplit takes a file's full path name (path) as a string
                in the form
                ...
                * ext keeps the dot preceding the extension (.c, .exe, etc.)
                ...
                to __fnsplit, except that is doesn't return a value.

                _splitpath (or __fnsplit) and _makepath are invertible; if you
                split a given path with _splitpath (or __fnsplit), then
                merge the resultant components with _makepath, you end up
                with path.

Return value    __fnsplit returns an integer (composed of five flags,
...
*---------------------------------------------------------------------*/
int _CType _FARFUNC __fnsplit(const char *pathP, char *driveP, char *dirP,
...
void _FARFUNC _splitpath(const char *pathP, char *driveP, char *dirP,
                         char *nameP, char *extP)
{
        (void)__fnsplit(pathP,driveP,dirP,nameP,extP);
}
