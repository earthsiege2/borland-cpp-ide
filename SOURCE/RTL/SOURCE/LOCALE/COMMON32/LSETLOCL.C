...
/*
    ALL FUNCTIONS ASSUME LOCALE IS LOCKED VIA _lsetlocale()
*/
...
char * _RTLENTRY _EXPFUNC _lsetlocale( int category, const char *locale )
{
    ...
    setlocale_result = __lsetlocale( category, locale );
...
*
*    locale:   locale name
*        e.g. "en_US.DOS437", "fr_CA.DOS850", "en_GB.DOS850@Euro", etc.
*
*    if locale is NULL:
...
char * __lsetlocale( int category, const char *locale )

{
    ...
    */
...

    {
        ...
        case LC_ALL:
            ReturnPtr = _pLocale->LocaleString;
            break;

        case LC_COLLATE:
            ReturnPtr = _pLocale->CollationCat.Header.CategoryName;
            break;

        case LC_CTYPE:
            ReturnPtr = _pLocale->CtypeCat.Header.CategoryName;
            break;

        case LC_MONETARY:
            ReturnPtr = _pLocale->MonetaryCat.Header.CategoryName;
            break;

        case LC_NUMERIC:
            ReturnPtr = _pLocale->NumericCat.Header.CategoryName;
            break;

        case LC_TIME:
            ReturnPtr = _pLocale->TimeCat.Header.CategoryName;
            break;

        case LC_MESSAGES:
            ReturnPtr = _pLocale->MessagesCat.Header.CategoryName;
            break;

        case LC_userdef:
            ReturnPtr = _pLocale->UserCat.Header.CategoryName;
            break;
    ...
    */
    ...
    if ( __dup_locale( &LibStatus.ptmplocale, &_pLocale ) == LOCALE_ERROR )
        return( NULL );
    ...
    */
    ...
    if ( ( _pLocale->LocaleHdr.nCharMinBytes != 1 ) ||
         ( _pLocale->LocaleHdr.nCharMaxBytes != 1 ) )
    {
    ...
    */
    ...
    while( nCats-- )
    {
        ...
        case LC_COLLATE:
            ...
            pClass =
                (char *) _OSMEMALLOC(
                _pLocale->CollationCat.CollateInfo.nTableSize, 1 );
            ...
            if (
                _OSFILEREAD(
                    ...
                    _pLocale->CollationCat.CollateInfo.nLevels ) ),
                    nbytes ) <= 0 )
           ...
           _pLocale->pcollate   = _strcoll;
           _pLocale->ptransform = _strxfrm;

            /* remove previous class and collation tables */
            if ( _pLocale->pClass != NULL )
                _OSMEMFREE( _pLocale->pClass );

            /* set new class and collation tables */
            _pLocale->pClass           = pClass;

            /* set locale pointers */
            _pLocale->pExpandTbl       = (struct Expansion    *) pExpansion;
            _pLocale->pCompressTbl     = (struct Compression  *) pCompression;
            _pLocale->pSubstitutionTbl = (struct Substitution *) pSubstitution;
        ...
        case LC_CTYPE:
            ...
            _pLocale->pToLower = (unsigned char *)
                ( &( (unsigned char) _pLocale->pCtype [ SBCS_CTYPE_TSIZE ] ) );
        ...
        case LC_userdef:
            ...
            if ( _pLocale->UserCat.UserInfo == NULL )
                break;
...
int __open_localelib( struct LocaleLibraryStatus *LibStatus )
{
    ...
    if ( __find_localelib( _LocaleLibName ) == OSFILE_ERROR )
        return( OSFILE_ERROR );
...

{
    ...
    switch( category )
   {
    ...
    case LC_COLLATE:
        ...
        cat_offset = _pLocale->LocaleHdr.CollationOffset;

        cat_info = (void *) &_pLocale->CollationCat.CollateInfo;

        cat_infosize = sizeof( struct Collate );

        cat_hdr = (void *) &_pLocale->CollationCat.Header;
    ...
    case LC_CTYPE:
        ...
        cat_offset = _pLocale->LocaleHdr.TypeOffset;

        cat_info = (void *) _pLocale->pCtype;

        cat_infosize = SBCS_CTYPE_CATSIZE;

        cat_hdr = (void *) &_pLocale->CtypeCat.Header;
    ...
    case LC_MONETARY:
        ...
        cat_offset = _pLocale->LocaleHdr.MonetaryOffset;

        cat_info = (void *) &_pLocale->MonetaryCat.MonetaryInfo;

        cat_infosize = sizeof( struct Monetary );

        cat_hdr = (void *) &_pLocale->MonetaryCat.Header;
     ...
     case LC_NUMERIC:
        ...
        cat_offset = _pLocale->LocaleHdr.NumericOffset;

        cat_info = (void *) &_pLocale->NumericCat.NumericInfo;

        cat_infosize = sizeof( struct Numeric );

        cat_hdr = (void *) &_pLocale->NumericCat.Header;
    ...
    case LC_TIME:
        ...
        cat_offset = _pLocale->LocaleHdr.TimeOffset;

        cat_info = (void *) &_pLocale->TimeCat.TimeDateInfo;

        cat_infosize = sizeof( struct TimeDate );

        cat_hdr = (void *) &_pLocale->TimeCat.Header;
    ...
    case LC_MESSAGES:
        ...
        cat_offset = _pLocale->LocaleHdr.MessagesOffset;

        cat_info = (void *) &_pLocale->MessagesCat.MessagesInfo;

        cat_infosize = sizeof( struct Messages );

        cat_hdr = (void *) &_pLocale->MessagesCat.Header;
    ...
    case LC_userdef:
        ...
        cat_offset = _pLocale->LocaleHdr.UserDefinitionsOffset;

        if ( _pLocale->UserCat.UserInfo == NULL )
            return( LOCALE_ERROR );

        cat_info = (void *) _pLocale->UserCat.UserInfo;

        /* size is unknown at this point */
        cat_infosize = 0;

        cat_hdr = (void *) &_pLocale->UserCat.Header;
    ...
    if ( ! ( _pLocale->LocaleHdr.CategoriesSupported & cat_mask ) )
        return( LOCALE_ERROR );
...

{
    ...
    if ( findfirst( localelib, &fileblock, 0 ) == 0 )
    {
        strcpy ( _LocaleLibName, localelib );
        return( OSFILE_NO_ERROR );
    }
    ...
    */
    ...
    {
        strcpy ( _LocaleLibName, tmp_pathname );
        return( OSFILE_NO_ERROR );
    }
