 ...
 *
 * function(s)
 *        _dostounix - converts date and time to UNIX time format
 *        _unixtodos - converts from UNIX-format time
 *-----------------------------------------------------------------------*/
...
Name            _dostounix - converts date and time to UNIX time format

Usage           long _dostounix(struct date *d, struct time *t);

Prototype in    nowhere, private function used by timecvt.c and _ftime.c
...
long _FARFUNC _dostounix(struct date *d, struct time *t)
{
        ...
        if (_daylight && __isDST( t->ti_hour, days, 0, d->da_year-1970))
                hours--;
        x += hours * 3600L;
...
Name            _unixtodos - converts from UNIX-format time

Usage           void _unixtodos(long time, struct date *d, struct time *t);

Prototype in    nowhere, private function used by timecvt.c
...
void _FARFUNC _unixtodos(long time, struct date *d, struct time *t)
{
        ...
        t->ti_hund = 0;
        t->ti_sec = (unsigned char)(time % 60);
        time /= 60;                             /* Time in minutes */
        t->ti_min = (unsigned char)(time % 60);
        ...
        if (_daylight && __isDST( (int)(time % 24), (int)(time / 24), 0, d->da_year-1970 ))
                time++;
        t->ti_hour = (unsigned char)(time % 24);
