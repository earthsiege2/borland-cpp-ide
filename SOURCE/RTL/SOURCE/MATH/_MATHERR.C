...
Description     __matherr serves as a focal  point for error handling in all
                ...
                __matherr does four things :
                        . It  uses its arguments  to fill out  an _exception
                          structure.

                        . It  calls   matherr  with  e,  a  pointer  to the
                          _exception  structure,  to   see  if  matherr  can
                          resolve the error.
                ...
                type field in the _exception  structure) onto an errno value
...
double _RTLENTRY _EXPFUNC __matherr(_mexcep why, char *fun, double *arg1p,
...
#else
        wsprintf (errMsg, "%s: %s error", fun, _mathwhy [why - 1]);
#endif
        switch (why)
        {
        case OVERFLOW:
