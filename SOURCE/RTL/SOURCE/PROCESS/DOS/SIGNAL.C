...
#include <_process.h>   /* Get definition for _abort() */
#include <errno.h>
#include <dos.h>
...
typedef void (_USERENTRY * CatcherPTR)();
...
static void interrupt Int23Catcher()
{
        ...
        if ((action = Dispatch[0]) != SIG_IGN)
        {
                ...
                setvect(0x23, OldInt23);  /* Set INT 23H to OLD handler */
...
/*+----------------------------------------------------------------------+
  ...
  +----------------------------------------------------------------------+*/
void (_USERENTRY *signal(sig, New))(int)
int sig;
CatcherPTR New;
