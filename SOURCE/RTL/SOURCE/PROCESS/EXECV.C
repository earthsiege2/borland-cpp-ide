 ...
 *        _wexecv - load and execute a program
 *-----------------------------------------------------------------------*/
...
Usage           int  execv(const char *pathname, char * const * argv);
                int  execv(const wchar_t *pathname, wchar_t * const * argv);
...
int _RTLENTRY _EXPFUNC _texecv(const _TCHAR *pathP, _TCHAR * const *argv)
{
        return (_tLoadProg(P_OVERLAY, pathP, argv, NULL, 0));
}
