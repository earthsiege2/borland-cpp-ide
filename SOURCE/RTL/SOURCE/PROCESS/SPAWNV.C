 ...
 *        _wspawnv - creates and runs child processes
 *-----------------------------------------------------------------------*/
...
Name            spawnv, _wspawnv - creates and runs child processes

Usage           #include <process.h>
                int spawnv(int modeF, const char *pathP, char * const * argv);
                int _wspawnv(int modeF, const _TCHAR *pathP, _TCHAR * const * argv);
...
int _RTLENTRY _EXPFUNC _tspawnv(int modeF, const _TCHAR *pathP,
                              _TCHAR * const * argv)
{
    /*
      ...
      a Func argument.
    */
        return _tLoadProg(modeF, pathP, argv, NULL, 0);
}
