 ...
 * Public variables
 */
int _setenvp__ = 0;                 /* satisfy ext. ref. generated by BCC32 */
static char *              envcopy; /* Our inital copy of the master table */
char ** _RTLENTRY _EXPDATA environ; /* pointer to array of env. strings */
int     _RTLENTRY _EXPDATA envsize; /* number of env strings allocated
...
Name            _extenvp - clean up environment memory block
...
static void _extenvp(void)
{
#pragma exit _extenvp 3
    if (envcopy)
        free ((void *)envcopy);
    if (environ)
        free ((void *)environ);
...
Name            _expandblock - expands the environ block to hold
                               more entries.
 ...
 */
    ...
    if (!environ)
    {
        /* For the first time called, get the strings from the master
           environment table: _osenv.
        */

        /* First, count the strings in the master environment.
         */
        for (total=0, n = 0, p = _osenv; (len = strlen(p)) != 0; n++, p += len + 1)
        ...
        */
        ...
        if ((envcopy = (char *) malloc (total+1)) == NULL)
            return 0;

        memcpy ((char *)envcopy, _osenv, total+1);
    }
    else
    {
        /* Otherwise, count the strings in the previous environ block.
         */
        for (n = 0, pt=environ; *pt ; n++, pt++)
        {
            if (strlen (*pt) == 0)
              n--;
        }
     ...
     * terminator.
     */
    if ((e = calloc((envsize+1),sizeof(char*))) == NULL)
        return 0;
    ...
    if (!environ)
    {
        for (n = 0, p = envcopy; (len = strlen(p)) != 0; n++, p += len + 1)
        {
            e[n] = p;
        }
    }
