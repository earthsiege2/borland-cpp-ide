 ...
 *        _wspawnl - creates and runs child processes
 *-----------------------------------------------------------------------*/
...
Name            spawnl, _wspawnl - creates and runs child processes

Usage           #include <process.h>
                int spawnl(int modeF,const char *pathP, const char *arg0,
                           const char *arg1, ..., const char *argn, NULL);
                int _wspawnl(int modeF,const wchar_t *pathP, const wchar_t *arg0,
...
int _RTLENTRY _EXPFUNC  _tspawnl(int modeF, const _TCHAR *pathP, const _TCHAR *arg0, ...)
{
    /*
      For OS/2 LoadProg always does a DosExecPgm, so it doesn't need
      a Func argument.
    */
        return _tLoadProg(modeF, pathP, &arg0, NULL, 0);
}
