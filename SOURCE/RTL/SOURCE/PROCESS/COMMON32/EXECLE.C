 ...
 *        _wexecle - load and execute a program
 *-----------------------------------------------------------------------*/
...
Usage           int  execle(const char *pathname, const char *arg0,
                           const char *arg1, ...,
                           const char *argn, NULL, const char *envp[]);
                int  execle(const wchar_t *pathname, const wchar_t *arg0,
                           const wchar_t *arg1, ...,
...
int _RTLENTRY _EXPFUNC _texecle(const _TCHAR *pathP, const _TCHAR *arg0,...)
{
    const _TCHAR   **p;
    ...
    for (p = &arg0; *p++ != NULL; );
    return (_tLoadProg(P_OVERLAY, pathP, &arg0, (const _TCHAR **)*p, 0));
}
