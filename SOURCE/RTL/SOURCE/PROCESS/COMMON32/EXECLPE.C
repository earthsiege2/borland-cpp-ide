 ...
 *        _wexeclpe - load and execute a program
 *-----------------------------------------------------------------------*/
...
Usage           int  execlpe(const char *pathname, const char *arg0,
                             const char *arg1, ...,
                             const char *argn, NULL, const char *envp[]);
                int  _wexeclpe(const wchar_t *pathname, const wchar_t *arg0,
                             const wchar_t *arg1, ...,
                             const wchar_t *argn, NULL, const wchar_t *envp[]);
...
int _RTLENTRY _EXPFUNC _texeclpe(const _TCHAR *pathP, const _TCHAR *arg0,...)
{
        const _TCHAR   **p;
        ...
        for (p = &arg0; *p++ != NULL; );
        return (_tLoadProg(P_OVERLAY, pathP, &arg0, (const _TCHAR **)*p, 1));
}
