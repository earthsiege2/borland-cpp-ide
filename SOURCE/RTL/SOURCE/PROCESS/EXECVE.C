 ...
 *        _wexecve - load and execute a program
 *-----------------------------------------------------------------------*/
...
Usage           int  execve(const char *pathname, char * const * argv,
                            char * const * envp);
                int  execve(const wchar_t *pathname, wchar_t * const * argv,
                            wchar_t * const * envp);
...
int _RTLENTRY _EXPFUNC _texecve(const _TCHAR *pathP, _TCHAR * const * argv,
                              _TCHAR * const * envV)
{
        return (_tLoadProg(P_OVERLAY, pathP, argv, envV, 0));
}
