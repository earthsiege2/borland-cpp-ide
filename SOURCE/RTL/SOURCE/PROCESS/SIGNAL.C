...
typedef void  (_USERENTRY *CatcherPTR3)(int,int, _PSIGCONTEXT);
                               ...
                               PVOID s)
{
    ...
    /* Create instance of extra WIN32 goodies to pass to user's handler
     */

    sc._pexecptionrecord = (PEXCEPTION_RECORD)p;
    sc._pcontext = r;
    sc._result =  XCPT_CONTINUE_EXECUTION;
	 ...
	 * out and never returns, no more exceptions could be processed.
        ...
        ((CatcherPTR3)Action)(IxGen[Index],FpeCode, &sc);   /* call handler */
        r->FloatSave.StatusWord &= ~SwMask;  /* clear exception bit */
    }
    else
        ((CatcherPTR3)Action) (IxGen[Index], 0, &sc);
    return (sc._result);
}
    ...
    if ((Action = Dispatch[Index]) != SIG_IGN)
    {
        ...
        else
        {
            ...
            if (SigType != SIGFPE)     /* Maybe default (MS) */
                Dispatch[Index] = SIG_DFL;
#endif
            FpeCode = ExplicitVal[Index];
            ((CatcherPTR3)Action)(SigType, FpeCode, NULL);
        }
    }
    return 0;
}
