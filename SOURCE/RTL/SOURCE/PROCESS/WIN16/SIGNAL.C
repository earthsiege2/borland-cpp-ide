...
/*+----------------------------------------------------------------------+
  ...
  +----------------------------------------------------------------------+*/
#ifdef _BUILDRTLDLL
_CatcherPTR _FARFUNC __signalDS(int sig, _CatcherPTR New, unsigned DS)
#else
_CatcherPTR _FARFUNC signal(int sig, _CatcherPTR New)
#endif
{
        register int    Index;
        _CatcherPTR      OldVal;
...
#ifdef _BUILDRTLDLL
                        Explicit = ExplicitVal[Index];
                        LocalDS = _DS;
                        _DS = SavedDS[Index];
                        (*action)(SigType, Explicit);
                        _DS = LocalDS;
#else
